...
- Updates `hugo.toml` with the current version
- Generates `static/version.json` for the in-IDE update checker
- Generates `static/updatePlugins.xml` — IntelliJ custom plugin repository XML (queries GitHub for ZIP filename via `gh`, includes full plugin description and change-notes from `release/notes/<version>.md`)
- Builds the Hugo site to `mcp-steroid-public/docs/`
...
- The `updatePlugins.xml` is generated by `website/scripts/generate-update-plugins-xml.py` (Python XML DOM API for proper escaping).
