...
- The `updatePlugins.xml` is generated by `website/scripts/generate-update-plugins-xml.py` (run via `uv`). It downloads the release ZIP, extracts `plugin.xml` from the JAR to get the exact version and `since-build`, and validates URL format. No fallbacks — errors are fatal.
