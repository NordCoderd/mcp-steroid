...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine("- **Use native Read/Grep/Glob tools for reading existing files** — do NOT use `steroid_execute_code` with VfsUtil.loadText() just to read an existing file that has not been modified. Each exec_code call compiles Kotlin (~8s overhead). Use exec_code only for: PSI queries, VFS writes, test execution, compile checks.")
