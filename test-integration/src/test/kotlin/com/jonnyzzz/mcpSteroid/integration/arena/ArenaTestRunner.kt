...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine("val result = CompletableDeferred<Boolean>()")
            ...
            appendLine("        val passed = r.isPassed(); val failed = r.getAllTests().count { it.isDefect }")
            ...
            appendLine("        conn.disconnect(); result.complete(passed)")
            appendLine("    }")
            ...
            appendLine("val passed = withTimeout(5.minutes) { result.await() }")
            ...
            appendLine("    object : TaskCallback { override fun onSuccess() { result.complete(true) }; override fun onFailure() { result.complete(false) } },")
            appendLine("    ProgressExecutionMode.IN_BACKGROUND_ASYNC, false)")
            appendLine("val ok = withTimeout(5.minutes) { result.await() }; println(\"Gradle result: success=\$ok\")")
            appendLine("```")
