...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine("  will ALWAYS fail at Spring context load with: 'Could not find a valid Docker environment'.")
            appendLine("  **How to handle Docker failures:**")
            appendLine("  1. Run `./mvnw test-compile -q` first. Exit 0 = your code is correct.")
            appendLine("  2. Then run the FAIL_TO_PASS test. If it fails ONLY due to Docker/Testcontainers")
            appendLine("     (ApplicationContext failure, 'Could not find a valid Docker environment'),")
            appendLine("     this is an **infrastructure constraint — NOT a code defect**. Your fix IS complete.")
            appendLine("  3. In that case: output `ARENA_FIX_APPLIED: yes` and document the Docker constraint")
            appendLine("     in ARENA_SUMMARY. Do NOT keep trying to work around Docker — it cannot be resolved.")
            appendLine("  4. If the test does NOT use @Testcontainers, you MUST fix it until it passes.")
            appendLine("- **Use `./mvnw` (Maven wrapper), not `mvn`** — the system `mvn` is not installed.")
            appendLine("  → **NEVER** print raw process output — Spring Boot test output exceeds 200k chars (startup + Flyway + traces).")
            appendLine("  → Always read with `takeLast()` to avoid MCP token limit errors:")
            appendLine("  ```kotlin")
            appendLine("  val process = ProcessBuilder(\"./mvnw\", \"test\", \"-Dtest=ClassName\", \"-Dspotless.check.skip=true\", \"-q\")")
            appendLine("      .directory(java.io.File(project.basePath!!)).redirectErrorStream(true).start()")
            appendLine("  val lines = process.inputStream.bufferedReader().readLines()")
            appendLine("  val exitCode = process.waitFor()")
            appendLine("  println(\"Exit: \$exitCode | lines: \${lines.size}\")")
            appendLine("  // Capture BOTH ends: Testcontainers/Spring context errors appear at START; BUILD FAILURE at END")
            appendLine("  println(lines.take(30).joinToString(\"\\n\"))")
            appendLine("  println(\"---\")")
            appendLine("  println(lines.takeLast(30).joinToString(\"\\n\"))")
            appendLine("  ```")
            appendLine("- Compile success (`test-compile`) is NOT the same as test pass. Always run the actual tests.")
            appendLine()
            appendLine("**⚡ MANDATORY FIRST STEP — Run FAIL_TO_PASS tests BEFORE writing any code**")
            appendLine()
            appendLine("   In a multi-agent arena, a previous agent slot may have already solved the problem.")
            appendLine("   Run the failing tests NOW. If they already pass, STOP — do NOT touch the code.")
            appendLine()
            if (testCase.buildSystem == "maven") {
                appendLine("   ```kotlin")
                appendLine("   // Replace TestClassName with the simple class name from the FAIL_TO_PASS list above")
                appendLine("   val proc = ProcessBuilder(\"./mvnw\", \"test\", \"-Dtest=TestClassName\", \"-Dspotless.check.skip=true\", \"-q\")")
                appendLine("       .directory(java.io.File(\"$projectDir\")).redirectErrorStream(true).start()")
                appendLine("   val lines = proc.inputStream.bufferedReader().readLines()")
                appendLine("   val exit = proc.waitFor()")
                appendLine("   println(\"Exit: \$exit | lines: \${lines.size}\")")
                appendLine("   println(lines.take(30).joinToString(\"\\n\"))")
                appendLine("   println(\"---\")")
                appendLine("   println(lines.takeLast(30).joinToString(\"\\n\"))")
                appendLine("   ```")
                ...
                appendLine("   ```kotlin")
                appendLine("   // Replace com.example.TestClass with the fully-qualified class from the FAIL_TO_PASS list")
                appendLine("   // Or use module targeting: \"./gradlew\", \":module-name:test\", \"--no-daemon\", \"-q\"")
                appendLine("   val proc = ProcessBuilder(\"./gradlew\", \"test\",")
                appendLine("       \"--tests\", \"com.example.TestClass\", \"--no-daemon\", \"-q\")")
                appendLine("       .directory(java.io.File(\"$projectDir\")).redirectErrorStream(true).start()")
                appendLine("   val lines = proc.inputStream.bufferedReader().readLines()")
                appendLine("   val exit = proc.waitFor()")
                appendLine("   println(\"Exit: \$exit | lines: \${lines.size}\")")
            ...
            appendLine("   → **Exit 0 and all FAIL_TO_PASS pass**: output `ARENA_FIX_APPLIED: no` and STOP. Do NOT modify any code.")
            appendLine("   → **Exit non-zero**: tests still fail → proceed with the fix workflow below.")
            appendLine()
            appendLine("0. **Check for pre-existing work before writing any code.** In a multi-agent arena, another")
            appendLine("   agent may have already modified files. Run this check first:")
            appendLine("   ```kotlin")
            appendLine("   val proc = ProcessBuilder(\"git\", \"diff\", \"HEAD\", \"--name-only\")")
            appendLine("       .directory(java.io.File(\"$projectDir\")).start()")
            appendLine("   val changed = proc.inputStream.bufferedReader().readText().trim()")
            appendLine("   println(if (changed.isEmpty()) \"No changes yet — clean slate\" else \"Already modified:\\n\$changed\")")
            appendLine("   ```")
            appendLine("   If files were already modified, review and extend them rather than overwrite.")
            appendLine()
            ...
            appendLine("   **Pre-implementation checklist (do this BEFORE writing any code):**")
            appendLine("   - List every HTTP endpoint the test calls (URL, method, expected status code)")
            appendLine("   - For each PUT/POST endpoint, check what the test asserts in the response body → determines service return type")
            appendLine("   - List every field name the test references in entities/DTOs → use EXACTLY those names, no extras")
            appendLine("   - Before adding DB columns: verify the test provides values for that column (if not, skip it)")
            appendLine("   - Check naming conventions: `PsiShortNamesCache.getInstance(project).allClassNames.toList()`")
            appendLine("   - Find next Flyway version: list files under src/main/resources/db/migration, use V(max+1)__")
            ...
            appendLine("5. **After creating/modifying files, verify compilation BEFORE running tests:**")
            appendLine("   ```kotlin")
            appendLine("   // Fast compile check — run this first to catch errors early (seconds, not minutes):")
            appendLine("   val vf = findProjectFile(\"src/main/java/com/example/YourNewClass.java\")!!")
            appendLine("   val problems = runInspectionsDirectly(vf)")
            appendLine("   if (problems.isEmpty()) println(\"OK: no compile errors\")")
            appendLine("   else problems.forEach { (id, descs) -> descs.forEach { println(\"[\$id] \${it.descriptionTemplate}\") } }")
            appendLine("   ```")
            appendLine("   **⚠️ Scope limitation**: `runInspectionsDirectly` checks only the ONE file you pass.")
            appendLine("   It does NOT catch errors in other files that depend on your changed signatures.")
            appendLine("   After modifying a DTO/command/entity, also check the service, controller, and mapper files.")
            appendLine("   Call `steroid_execute_feedback` after this step to record the result.")
            appendLine("6. Run the failing tests to confirm they pass (only after compile check succeeds)")
            appendLine()
        ...
        appendLine()
        appendLine("**COMPLETION GATE** — Before outputting `ARENA_FIX_APPLIED: yes`:")
        if (testCase.buildSystem == "maven") {
            appendLine("1. Run the FAIL_TO_PASS test(s): `./mvnw test -Dtest=TestClassName -Dspotless.check.skip=true -q`")
            appendLine("   (Replace TestClassName with the simple class name, e.g. `NotificationIntegrationTest`)")
            appendLine("2. Run the full test suite to detect regressions: `./mvnw test -Dspotless.check.skip=true -q`")
        } else {
            appendLine("1. Run the FAIL_TO_PASS test(s): `./gradlew test --tests \"com.example.TestClass\" --no-daemon -q`")
            appendLine("   (Use fully-qualified class name, e.g. `com.example.microshop.product.ProductServiceTest`)")
            appendLine("   Or run by module: `./gradlew :module-name:test --no-daemon -q`")
            appendLine("2. Run the full test suite to detect regressions: `./gradlew test --no-daemon -q`")
        }
        appendLine("   If any PASS_TO_PASS test now fails because of your change, fix it before claiming success.")
        appendLine("   (Skip this step only if the project has no PASS_TO_PASS tests listed above.)")
        appendLine("3. Confirm they pass. Compile-only verification is NOT sufficient.")
        appendLine("   Runtime mismatches (wrong return types, missing fields) are invisible to the compiler.")
        appendLine("4. If tests fail ONLY because of Docker/Testcontainers unavailability (ApplicationContext failure,")
        appendLine("   'Could not find a valid Docker environment'), and `test-compile` exit 0 confirms your code")
        appendLine("   compiles correctly — claim `ARENA_FIX_APPLIED: yes` and document the constraint in ARENA_SUMMARY.")
        appendLine()
