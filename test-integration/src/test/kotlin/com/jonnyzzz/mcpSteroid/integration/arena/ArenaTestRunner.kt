...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        appendLine("- **Docker is NOT available.** Tests that use `@Testcontainers` or require a running database")
        appendLine("  will always fail with 'Could not find a valid Docker environment'.")
        appendLine("  If tests fail ONLY for this reason and `test-compile` exits 0, your fix IS complete.")
        appendLine("- **Multi-agent environment**: another agent slot may have already modified files.")
        appendLine("  Check `git diff HEAD --name-only` before writing any code.")
        ...
        appendLine()
        appendLine("If tests fail ONLY because of Docker/Testcontainers unavailability, still output")
        appendLine("`ARENA_FIX_APPLIED: yes` — document the Docker constraint in ARENA_SUMMARY.")
    }
