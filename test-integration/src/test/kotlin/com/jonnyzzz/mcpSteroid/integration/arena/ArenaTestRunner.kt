...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        appendLine("    Use `runInspectionsDirectly` for compile verification of your changes.")
        appendLine("    Then identify and run **unit tests** (tests that do NOT use `@SpringBootTest` or `@Testcontainers`) —")
        appendLine("    these run without Docker and verify your logic. Find them with:")
        appendLine("    ```kotlin")
        appendLine("    import com.intellij.psi.search.FilenameIndex")
        appendLine("    import com.intellij.psi.search.GlobalSearchScope")
        appendLine("    val unitTests = readAction {")
        appendLine("        FilenameIndex.getAllFilesByExt(project, \"java\", GlobalSearchScope.projectScope(project))")
        appendLine("            .filter { it.path.contains(\"/test/\") }")
        appendLine("            .filter { vf ->")
        appendLine("                val txt = com.intellij.openapi.vfs.VfsUtil.loadText(vf)")
        appendLine("                txt.contains(\"@Test\") && !txt.contains(\"@SpringBootTest\") && !txt.contains(\"@Testcontainers\")")
        appendLine("            }.map { it.name }")
        appendLine("    }")
        appendLine("    println(\"Unit tests (no Docker needed): \" + unitTests.joinToString(\", \"))")
        appendLine("    ```")
        appendLine("    Run those unit tests via `./mvnw test -Dtest=<UnitTestClass>`. Report `ARENA_FIX_APPLIED: yes` once they pass.")
        appendLine("    Do NOT investigate Docker further.")
        ...
        if (testCase.buildSystem == "maven") {
            appendLine("- **Maven pom.xml workflow** (if you need to add/change dependencies):")
            appendLine("  1. Edit `pom.xml` using the native `Edit` tool")
            appendLine("  2. Immediately sync via `steroid_execute_code` and await completion:")
            appendLine("  ```kotlin")
            appendLine("  import org.jetbrains.idea.maven.project.MavenProjectsManager")
            appendLine("  import org.jetbrains.idea.maven.project.MavenSyncSpec")
            appendLine("  import com.intellij.platform.backend.observation.Observation")
            appendLine("  MavenProjectsManager.getInstance(project)")
            appendLine("      .scheduleUpdateAllMavenProjects(MavenSyncSpec.incremental(\"post-pom-edit\"))")
            appendLine("  Observation.awaitConfiguration(project)")
            appendLine("  println(\"Maven sync complete\")")
            appendLine("  ```")
            appendLine("  3. Only then run `runInspectionsDirectly` or `./mvnw test` — without step 2,")
            appendLine("     new imports show `cannot resolve symbol` errors from undownloaded dependencies.")
            appendLine("- **After creating multiple new files** via `writeAction { VfsUtil.saveText(...) }`:")
            appendLine("  The NEXT `steroid_execute_code` call automatically waits for PSI re-indexing.")
            appendLine("  Only add an explicit `waitForSmartMode()` if inspecting in the SAME call as file creation.")
        }
        ...
        appendLine()
        appendLine("## Inspection Signal Semantics")
        appendLine()
        appendLine("When `runInspectionsDirectly` reports warnings, apply this classification:")
        appendLine("- **`[ConstantValue] Value ... is always 'null'`** on a DTO accessor call (e.g. `dto.releasedAt()`) in a test file")
        appendLine("  → **CRITICAL BUG**: the DTO record is missing that component. This is a guaranteed runtime NPE/assertion failure.")
        appendLine("  Do NOT dismiss as 'pre-existing noise'. Investigate: read the DTO source, compare its record components")
        appendLine("  against what the test calls, and add the missing field(s).")
        appendLine("- **`[ClassCanBeRecord]`** on a new DTO/data class → **REQUIRED**: convert to Java record.")
        appendLine("- **`[ClassEscapesItsScope]`** on Spring beans → **Expected, non-blocking**: ignore.")
        appendLine("- **`[GrazieInspectionRunner]`**, **`[DeprecatedIsStillUsed]`** → **Low priority**: cosmetic only.")
        appendLine()
