...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine()
            appendLine("**Verify compile ONCE with runInspectionsDirectly, not ./mvnw test-compile**: Each Maven")
            appendLine("cold-start takes ~90s. After creating files, run `runInspectionsDirectly(vf)` (~5s) for each")
            appendLine("new file. If it returns no errors, trust it — do NOT add a redundant `./mvnw test-compile`.")
            appendLine("Only run `./mvnw test -Dtest=ClassName` for final FAIL_TO_PASS verification.")
            appendLine()
            ...
            appendLine("blocking the latch for up to **600 seconds**. Multiple agents hitting this in the same run = 1800s wasted.")
            ...
            appendLine("Without it, a Maven project-reimport dialog can silently block for the full 10-minute latch timeout (600s wasted).")
            ...
            appendLine("latch.await(10, TimeUnit.MINUTES)")
            ...
            appendLine("latch.await(10, TimeUnit.MINUTES); println(\"Gradle result: success=\$ok\")")
            appendLine("```")
