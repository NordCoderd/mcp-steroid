...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine("  → If a FAIL_TO_PASS test fails because of Docker (not because of your code), document it.")
            appendLine("  → Check: does the test use @Testcontainers? If yes, Docker is the reason — not your code.")
            appendLine("  → If the test does NOT use @Testcontainers, you MUST fix it until it passes.")
        ...
        appendLine("4. If tests fail because of Docker/Testcontainers unavailability (not because of your code),")
        appendLine("   document that in ARENA_SUMMARY but do NOT claim ARENA_FIX_APPLIED: yes.")
        appendLine()
