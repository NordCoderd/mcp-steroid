...
class DebuggerDemoTest {
    ...
    private fun runJonnyzzzDebugDemo(agentName: KProperty1<AiAgentDriver, AiAgentSession>) {
        ...
        val prompt = buildString {
            ...
            appendLine("2. Read the test to find the source class under test, then debug the failing test")
            appendLine("3. Set a breakpoint in the function being tested and run the debugger")
            appendLine("4. Observe variable values during execution to understand why the assertion fails")
            appendLine()
            appendLine("Read `mcp-steroid://skill/debugger-skill` to learn how to use the debugger APIs.")
            appendLine("It links to individual resources with complete, copy-paste-ready code for each step.")
            appendLine()
            ...
            appendLine("ROOT_CAUSE: <explain the root cause of the filter bug — what condition is wrong and what effect does it have on the returned items>")
            ...
            appendLine("## Rules")
            appendLine()
            appendLine("- You MUST use the debugger (set breakpoints, evaluate variables)")
            appendLine("- Do NOT use screenshots or UI input tools")
            appendLine("- Read MCP debugger resources for API patterns -- do not invent API calls")
        }
