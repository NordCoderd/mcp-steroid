...
class RiderDebuggerTest {
    ...
    private fun runRiderDebugDemo(agentName: KProperty1<AiAgentDriver, AiAgentSession>) {
        ...
        val prompt = buildString {
            ...
            appendLine("Do NOT just read source code and guess.")
            appendLine()
            appendLine("Browse the MCP resources to learn how to use the debugger and run tests.")
            appendLine()
            ...
            appendLine("DEBUGGER_EVIDENCE: <variable values observed BEFORE and AFTER the buggy line>")
            appendLine()
            appendLine("Also print BEFORE_VALUE and AFTER_VALUE markers when evaluating variables.")
            appendLine()
            appendLine("## Rules")
            appendLine()
            appendLine("- You MUST use the debugger (breakpoints + variable evaluation)")
