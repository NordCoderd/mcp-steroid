...
class RiderDebuggerTest {
    ...
    private fun runRiderDebugDemo(agentName: KProperty1<AiAgentDriver, AiAgentSession>) {
        ...
        val prompt = buildString {
            ...
            appendLine()
            appendLine("1. Find `LeaderboardTests.cs` in the project — it is an NUnit test class in the DemoRider.Tests project")
            appendLine("2. Find the corresponding `Player.cs` source file that contains the `Leaderboard.GetLeaderboard()` method")
            appendLine("3. Use the debugger to run the failing test and set a breakpoint inside the `GetLeaderboard()` method")
            appendLine("4. Step through the code and observe variable values before and after the OrderByDescending call")
            ...
            appendLine()
            appendLine("IMPORTANT: You must debug the NUnit test (LeaderboardTests), not run Program.cs.")
            appendLine("Use the IDE's test runner and debugger APIs via `steroid_execute_code`.")
            appendLine("Do NOT reference JUnitConfiguration — this is a .NET project, not Java.")
            appendLine("Explore available run configuration types for .NET test debugging.")
            appendLine()
