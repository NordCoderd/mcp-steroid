...
File:   docs/04-evaluation/020_prompts/010_evaluation-prompt.md
Commit: HEAD as of 2026-04-22
License: MIT (see https://github.com/oraios/serena/blob/main/LICENSE)

The text below (starting at the first `#` heading, everything up to and including
the "## What I am not looking for" section) is reproduced VERBATIM from the Serena
project's self-evaluation prompt. MCP Steroid stores a local copy so that the
integration tests that exercise the prompt do not depend on upstream availability
or revisions. Do not edit the body of the prompt — if Serena updates its prompt and
we want to consume the new version, refresh this file against the upstream source
and update the "Commit" line above.
