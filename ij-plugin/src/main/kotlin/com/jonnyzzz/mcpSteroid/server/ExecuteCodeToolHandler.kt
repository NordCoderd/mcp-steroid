...
class ExecuteCodeToolHandler : McpRegistrar {
    private val toolDescription get() = buildString {
        ...
        appendLine()
        appendLine("**Prefer VfsUtil over native Read tool**: For project source files, use `VfsUtil.loadText(findProjectFile(\"path/File.java\")!!)` inside exec_code instead of the Claude native Read tool — reads the IDE's in-memory VFS (including unsaved modifications from prior writeAction calls). Native Read bypasses VFS and may return stale content.")
        appendLine()
