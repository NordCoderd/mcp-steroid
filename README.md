...
The `code` you send is the **suspend script body**. You do **not** need an `execute { }` wrapper. `waitForSmartMode()` is called automatically before your script starts; call it again only if you trigger indexing mid-script. Never use `runBlocking`:
...
// For read/write actions, use IntelliJ's coroutine-aware APIs (readAction/writeAction)
...
**Default Imports** (auto-added; explicit imports are optional):
```kotlin
...
**Note:** `readAction`, `writeAction`, and `smartReadAction` are built into `McpScriptContext` - no imports needed! Explicit imports are optional.
