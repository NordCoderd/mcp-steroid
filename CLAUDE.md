...
- **BANNED: detecting failures and skipping tests.** Tests exist to show problems. Never add `try { } catch { skip() }`, `TestAbortedException` on error detection, `Assumptions.assumeTrue(isAvailable)`, or any pattern that turns a real failure into a skip. The only acceptable test skip is at the **Gradle task level** (`enabled = !condition`) when an entire test suite is structurally incompatible with the platform (e.g., native-only tests disabled on an OS that cannot run them). Individual test-level runtime skips that hide failures are forbidden. **Single documented exception: Gemini API key on CI** — the TeamCity server has no Gemini token configured and there is no plan to add one, so Gemini-using tests are skipped when `GEMINI_API_KEY`/`GOOGLE_API_KEY`/`~/.vertex` are all unset. Two complementary mechanisms cover this: (a) JUnit 5 / Jupiter tests (e.g. `:test-helper:test` `DockerGeminiProgressTest`) get an `AssumptionViolatedException` from `DockerGeminiSession.Companion.skipTestWhenKeyMissing=true` (see `test-helper/.../AISessionBase.kt`) — Jupiter reports them as ignored. (b) `BasePlatformTestCase`-backed tests (e.g. `:ij-plugin:integrationTest` `CliGeminiIntegrationTest`) are excluded via a **Gradle-task-level filter** in `ij-plugin/build.gradle.kts` because the JUnit 3↔4 bridge in `OldTestClassAdaptingListener` converts `AssumptionViolatedException` into a failure rather than honoring it as a skip. Both mechanisms still throw / fail when the key is present but is an unresolved `%credentialsJSON:…%` reference — that case is a real TC misconfiguration that must stay visible. Do not extend this opt-in to other agents (Anthropic / OpenAI keys ARE configured on TC; their tests must keep failing if the key disappears).
