...
- **BANNED: detecting failures and skipping tests.** Tests exist to show problems. Never add `try { } catch { skip() }`, `TestAbortedException` on error detection, `Assumptions.assumeTrue(isAvailable)`, or any pattern that turns a real failure into a skip. The only acceptable test skip is at the **Gradle task level** (`enabled = !condition`) when an entire test suite is structurally incompatible with the platform (e.g., native-only tests disabled on an OS that cannot run them). Individual test-level runtime skips that hide failures are forbidden. **Single documented exception: Gemini API key on CI** — `DockerGeminiSession.Companion` opts into `skipTestWhenKeyMissing=true` (see `test-helper/.../AISessionBase.kt`). The TeamCity server has no Gemini token configured and there is no plan to add one, so Gemini-using tests throw `AssumptionViolatedException` and are reported as ignored when `GEMINI_API_KEY`/`GOOGLE_API_KEY`/`~/.vertex` are all unset. The unresolved-TC-reference branch (`%credentialsJSON:…%`) still throws — that case remains a real misconfiguration. Do not extend this opt-in to other agents (Anthropic / OpenAI keys ARE configured on TC; their tests must keep failing if the key disappears).
