...
**Canonical location:** `docs/root/CLAUDE.md`. Root `CLAUDE.md` and `AGENTS.md` mirror this file for compatibility.
Edit this file when updating guidance (root stubs point here).

**IMPORTANT: Re-read this file often as it changes frequently. Instructions here override default behavior.**

Make sure you never include AI as co-author for commits. You must never mention AI in the commit messages too.
...
- [AGENT-STEROID-GUIDE.md](../guides/AGENT-STEROID-GUIDE.md) - **IntelliJ API usage guide for LLM agents** - read this to become a power user
- [Plan.md](../archive/Plan.md) - Implementation plan and phases (archived)
- [DEFERRED-FEATURES.md](../future/DEFERRED-FEATURES.md) - Open questions and design decisions
- [Discussions.md](../archive/Discussions.md) - Design discussions and decisions from Q&A sessions (archived)
...
- **MCP Server**: Internal MCP implementation aligned with spec 2025-11-25, using Ktor 3.1.0 (CIO + SSE)
...
- **MCP Server**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/server/SteroidsMcpServer.kt` - Ktor-based MCP server with SSE transport
- **Progress Reporting**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/server/McpProgressReporter.kt` - progress callback interface (NoOpProgressReporter for tests)
- **Execution**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/execution/ExecutionManager.kt` - manages script execution lifecycle
- **Code Evaluation**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/execution/CodeEvalManager.kt` - handles script compilation and captures the script body as a runnable block
- **Script Execution**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/execution/ScriptExecutor.kt` - runs the captured script body block with timeout
- **Script Context**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/execution/McpScriptContext.kt` / `McpScriptContextImpl.kt` - runtime context for scripts
- **Storage**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/storage/ExecutionStorage.kt` - append-only file-based storage (for logging/debugging)
- **Review**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/review/ReviewManager.kt` - human review workflow
- **Skills**: `src/main/kotlin/com/jonnyzzz/mcpSteroid/server/SkillCatalog.kt` / `SkillPromptRegistrar.kt` - skill catalog and prompt registration
...
3. **Progress is opt-in**: Tool handlers emit progress via `McpProgressReporter`; tests typically use `NoOpProgressReporter`.
...
│   ├── SkillCatalog.kt                    # skill catalog and metadata
│   ├── SkillPromptRegistrar.kt            # MCP prompt registration for skills
│   ├── SkillResourceHandler.kt            # skill resources handler
│   ├── SkillResources.kt                  # skill resource metadata
...
│   └── McpRootsService.kt                 # Roots capability handling
├── execution/
...
│   ├── McpScriptBuilder.kt        # Script block assembly
...
├── demo/
│   ├── DemoModeService.kt         # Demo mode state + events
│   └── DemoOverlayPanel.kt        # Overlay UI
├── ocr/
...
- `build.gradle.kts`: Plugin configuration using `intellijPlatform` DSL (e.g. `intellijIdeaUltimate("2025.3")`)
...
- Registry keys:
  ...
  - `mcp.steroid.demo.opacity`: Overlay background opacity (0-100)
  - `mcp.steroid.demo.focusFrame`: Bring project frame to front when showing overlay
