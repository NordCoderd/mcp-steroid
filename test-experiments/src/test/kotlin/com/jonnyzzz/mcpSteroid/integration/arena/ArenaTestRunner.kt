...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine("- **Native file read budget**: After the first `steroid_execute_code` VCS check, use at most **10 Read/Glob/Grep calls total** before writing your first edit. The test patch already shows what each test expects — read only the production files that directly correspond to modified test files, then start implementing. Do NOT read every service, repository, entity, and config file across all modules before writing a single line of code. 10 reads → first edit. Bias to action.")
