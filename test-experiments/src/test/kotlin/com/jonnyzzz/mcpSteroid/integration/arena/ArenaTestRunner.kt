...
class ArenaTestRunner(
    ...
    fun buildPrompt(testCase: DpaiaTestCase, projectDir: String, withMcp: Boolean = true): String = buildString {
        ...
        if (withMcp) {
            ...
            appendLine("- **Multi-site edits → `applyPatch` inside ONE `steroid_execute_code`**. When you are about to chain 2+ native `Edit` calls (same file or N files), STOP and batch them:")
            appendLine("  ```kotlin")
            appendLine("  applyPatch {")
            appendLine("    hunk(\"/abs/path/A.java\", \"oldA1\", \"newA1\")")
            appendLine("    hunk(\"/abs/path/A.java\", \"oldA2\", \"newA2\")")
            appendLine("    hunk(\"/abs/path/B.java\", \"oldB\",  \"newB\")")
            appendLine("  }")
            appendLine("  ```")
            appendLine("  One undoable command, pre-flight validation, PSI-consistent. For the SAME old→new applied to N files, use `listOf(paths).forEach { hunk(it, old, new) }` so the text ships once. Shortest unique anchor (30–60 chars) is enough — pre-flight rejects non-unique matches with `ApplyPatchException`. Native `Edit` remains valid ONLY for a single literal substitution in a single file.")
