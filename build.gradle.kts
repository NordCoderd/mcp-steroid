...
val generateMetadata by tasks.registering {
    ...
    val outputFile = outputDir.map { it.file("com/jonnyzzz/intellij/mcp/PluginMetadata.kt") }
    ...
    doLast {
        fun String.toEncodedSequence(): String {
            val codes = reversed().map { it.code }
        ...
        val content = """
            ...
            |    return sequence {
            |        $versionEncoded
            |    }.map { it.toChar() }.toList().reversed().joinToString("")
            |}
            |
            |/**
        ...
        println("Generated PluginMetadata.kt with:")
        println("  Version: $version")
        println("  Time Bomb: $TIME_BOMB")
    }
}
